// jQuery Inline Footnotes v1.0.2
  ...
  (function($) {
    $.inlineFootnote = function(el, options) {
      ...
      this.openBox = __bind(function(event) {
        ...
        if (!this.box) {
          footnoteContent = $("[id='" + this.footnoteId + "']").children().filter(":not('" + this.options.hideFromContent + "')");
          linkOffset = this.el.offset();
          this.box = $("<div />", {
            id: this.options.boxId,
            html: footnoteContent.clone().find(this.options.hideFromContent).remove().end(),
            css: {
              position: "absolute",
              top: linkOffset.top,
